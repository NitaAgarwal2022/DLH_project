...
def feature_hcpcs(cohort_output,version_path):
    print("[EXTRACTING HCPCS DATA]")
    print(cohort_output)
    print(version_path)
    hcpcs_path="./"+version_path+"/hosp/hcpcsevents.csv.gz"
    cohort_file_path='./data/cohort/'+cohort_output+'.csv.gz'
    
    adm = pd.read_csv(cohort_file_path, usecols=['hadm_id', 'admittime'], parse_dates = ['admittime'])
    hcpcs = pd.read_csv(hcpcs_path, compression='gzip', usecols=['subject_id', 'hadm_id', 'hcpcs_cd', 'chartdate', 'short_description'], parse_dates = ['chartdate'])
    hcpcs = hcpcs.merge(adm, left_on = 'hadm_id', right_on = 'hadm_id', how = 'inner')
    hcpcs['chart_time_from_admit'] = hcpcs['chartdate'] - hcpcs['admittime']
        
    print("Number of unique type of hcpcs code: ", hcpcs.hcpcs_cd.nunique())
    print("Total number of rows: ", hcpcs.shape[0])
    print("# Admissions:  ", hcpcs.hadm_id.nunique())
    
    hcpcs[['subject_id', 'hadm_id', 'hcpcs_cd','short_description', 'chartdate', 'admittime', 'chart_time_from_admit']].to_csv("./data/features/preproc_hcpcs.csv.gz", compression='gzip', index=False)
        
    print("[SUCCESSFULLY SAVED HCPCS DATA]")
